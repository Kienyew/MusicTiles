...
{
    return (min + ((float)GetRandomValue(0, __INT_MAX__) / __INT_MAX__) * (max - min));
}
...
// data will be copied.
static void list_append_tail(List* list, const void* data, size_t data_size)
{
    if (list->size == 0)
    {
...
static void* list_pop_head(List* list)
{
...
// list_pop_tail should be declared in general, but I didn't used that in overall, so I omitted it to reduce code size.
// static void* list_pop_tail(List* list);

// User is responsible to free the returned pointer
static void* list_pop_node(List* list, ListNode* target)
{
...
static void list_clear(List* list, void(*free_function)(void*))
{
    if (list == NULL) return;
    while (list->size > 0)
...
static bool register_at_frame(unsigned int frame, void(*f)(void*), void* data, size_t data_size)
{
    ...
    FrameRegistration registration = {
        .function = f,
        .frame    = frame,
        .data     = memcpy(malloc(data_size), data, data_size)
    };
...
void register_animate_text(unsigned int frame, const char* text, int fontsize, Color fontcolor, float start_x, float start_y)
{
    ...
    for (int i = 0; i < ANIMATE_TEXT_DURATION; ++i)
    {
        __frame_register_animate_text_t data = { 
            .textbox = animation_texts.tail->data,
            .dx      = -2*cosf(radians),
            .dy      = -2*sinf(radians)
        };
...
void updateGame()
{
    ...
    for (int line = 0; line < LINE; ++line)
    {
        ...
        if (IsKeyPressed(touch_blocks[line].key))
        {
            ...
            {
                MusicNote* music_note = (MusicNote*)node->data;
                Rectangle touch_rect = {
                    .x      = touch_blocks[line].x,
                ...
                Rectangle music_note_rect = {
                    .x      = music_note->x,
                    .y      = music_note->y,
                    .width  = music_note->width,
                    .height = music_note->height
                };
...
void drawGame()
{
    ...
    for (int line = 0; line < LINE; ++line)
    {
        ...
        DrawLineEx((Vector2) { LEFT_MARGIN + (line*LINE_WIDTH) + (line*MARGIN_BETWEEN_LINE), 0 },
                   (Vector2) { LEFT_MARGIN + (line*LINE_WIDTH) + (line*MARGIN_BETWEEN_LINE), SCREEN_HEIGHT - TOUCH_BLOCK_MARGIN_BOTTOM },
                   SIDE_LINE_WIDTH,
                   SIDE_LINE_COLOR);

        // right side
        DrawLineEx((Vector2) { LEFT_MARGIN + LINE_WIDTH + (line*LINE_WIDTH) + (line*MARGIN_BETWEEN_LINE), 0 },
                   (Vector2) { LEFT_MARGIN + LINE_WIDTH + (line*LINE_WIDTH) + (line*MARGIN_BETWEEN_LINE), SCREEN_HEIGHT - TOUCH_BLOCK_MARGIN_BOTTOM },
                   SIDE_LINE_WIDTH,
                   SIDE_LINE_COLOR);
    }
    ...
    for (int line = 0; line < LINE; ++line)
        for (ListNode* node = music_note_lists[line].head; node != NULL; node = node->prev)
        {
            DrawRectangle(((MusicNote*)node->data)->x,
                          ((MusicNote*)node->data)->y,
                          ((MusicNote*)node->data)->width,
                          ((MusicNote*)node->data)->height,
                          ((MusicNote*)node->data)->color);
        }

    // --- touch blocks ---
    for (int line = 0; line < LINE; ++line)
    {
        DrawRectangle(touch_blocks[line].x,
                      touch_blocks[line].y,
                      touch_blocks[line].width,
                      touch_blocks[line].height,
                      touch_blocks[line].color);

        // text on touch block
        char text[2] = { touch_blocks[line].key, '\0' };
        DrawText(text,
                 touch_blocks[line].x + TOUCH_BLOCK_FONT_SIZE / 2,
                 touch_blocks[line].y + TOUCH_BLOCK_FONT_SIZE / 2,
                 TOUCH_BLOCK_FONT_SIZE,
                 TOUCH_BLOCK_FONT_COLOR);
